...
COMPETITIONS = [
    ...
    {'id':'GER-Bundesliga','name':'Bundesliga','file':'bundesliga.svg','source':'wikimedia_commons','source_file':'Bundesliga_logo.svg','aliases':['Bundesliga','GER-Bundesliga']},
...
def main() -> None:
    ...
    manifest = {
        ...
        'season':'2026/27',
        'counts':{'requested':len(COMPETITIONS),'resolved':len(manifest_entries),'unresolved':len(unresolved)},
        'competitions':manifest_entries,
    }
    (OUT/'manifest.json').write_text(json.dumps(manifest,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
    (OUT/'unresolved.json').write_text(json.dumps(unresolved,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
